...
public sealed class CardHandSystem : EntitySystem
{
    ...
    private bool TryGetCardLayer(EntityUid card, out SpriteComponent.Layer? layer)
    {
        ...
        if (!TryComp(card, out SpriteComponent? cardSprite)
            || !_spriteSystem.TryGetLayer((card, cardSprite), 0, out var l, false))
            return false;
    ...
    private void UpdateSprite(EntityUid uid, CardHandComponent comp)
    {
        ...
        if (cardCount <= 0)
        {
            ...
            _spriteSystem.LayerSetVisible((uid, sprite), 0, true);
            _spriteSystem.LayerSetRsiState((uid, sprite), 0, "singlecard_down_black");
            _spriteSystem.LayerSetOffset((uid, sprite), 0, new Vector2(0f, 0f));
            _spriteSystem.LayerSetScale((uid, sprite), 0, new Vector2(1f, 1f));
        }
        else if (cardCount == 1)
        {
            _cardSpriteSystem.TryHandleLayerConfiguration(
                ...
                {
                    _spriteSystem.LayerSetRotation((uid, sprite), layerIndex, Angle.FromDegrees(0));
                    _spriteSystem.LayerSetOffset((uid, sprite), layerIndex, new Vector2(0, 0.10f));
                    _spriteSystem.LayerSetScale((uid, sprite), layerIndex, new Vector2(comp.Scale, comp.Scale));
                    return true;
                }
            );
        }
        else
        {
            ...
            _cardSpriteSystem.TryHandleLayerConfiguration(
                ...
                (sprt, cardIndex, layerIndex) =>
                {
                    ...
                    _spriteSystem.LayerSetRotation((uid, sprite), layerIndex, Angle.FromDegrees(-angle));
                    _spriteSystem.LayerSetOffset((uid, sprite), layerIndex, new Vector2(x, y));
                    _spriteSystem.LayerSetScale((uid, sprite), layerIndex, new Vector2(comp.Scale, comp.Scale));
                    return true;
                }
            );
